...
Three simulations, each a different flavor of emergence: simple local rules producing complex global behavior.
...
~400 lines of C total. Each simulation is one C file and one JS file.
...
**Toroidal boundaries.** Every grid wraps — modular arithmetic on all edge access. Boids agents wrap their floating-point positions; Langton and Oscillators wrap grid indices.

**Double-buffering.** Oscillators uses read/write buffer pairs, swapped after each step, so the diffusion reads stable data while writing updates.
...
**Trail visualization.** Boids and Langton use heat/trail maps that decay over time, turning agent paths into glowing traces. All three use warm-toned color ramps on dark backgrounds.
...
**C, not Rust/Zig** — ~400 lines of arithmetic on flat arrays. No allocator, no strings, no error handling. C is the right tool.
